...
- Responsive AVIF hero images with an image preload and high fetch priority; resized lossless WebP event logos and explicit dimensions; font-display swap; short-lived header-query caching. Both the map and its CSS load only near the viewport. The map container has a labelled region role.
